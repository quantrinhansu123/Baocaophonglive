...
export const StoreManager: React.FC = () => {
    ...
    const handleAddStore = async () => {
        ...
        try {
            ...
            // Nếu là của mình (own) thì chỉ lưu personnelIds, không lưu partnerId
            const storeData: any = {
                name: storeNameToAdd
            };
            
            if (newStorePartnerId && newStorePartnerId !== 'own') {
    ...
    const handleStartEdit = (store: Store) => {
        ...
        // Nếu không có partnerId, hiển thị "Của mình" (own)
        setNewStorePartnerId(store.partnerId || 'own');
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] overflow-y-auto">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md m-4 p-6 relative">
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); editingStore ? handleUpdateStore() : handleAddStore(); }} className="space-y-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setNewStorePartnerId(selectedValue);
                                        if (selectedValue && selectedValue !== 'own') {
                                            // Chọn đối tác: xóa nhân sự phụ trách
                                            setNewStorePersonnelIds([]);
                                        }
                                        // Nếu chọn "Của mình" (own) hoặc không chọn gì (''), giữ nguyên personnelIds
                                    }}
                            ...
                            {(!newStorePartnerId || newStorePartnerId === 'own') && (
                                <div>
