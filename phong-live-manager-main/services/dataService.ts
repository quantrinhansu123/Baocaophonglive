import { HostRevenue, VideoConfigItem, Store, AdShiftData, VideoMetric, LiveReport, Personnel, Partner, MenuPermission, TeamCDDailyReport, Expense } from '../types';
...
// --- TEAM CD DAILY REPORTS ---

export const fetchTeamCDDailyReports = async (): Promise<TeamCDDailyReport[]> => {
  let reports: TeamCDDailyReport[] = [];
  try {
    const response = await fetch(`${FIREBASE_URL}/team_cd_daily_reports.json`);
    if (response.ok) {
      const data = await response.json();
      if (data) {
        reports = Object.keys(data).map(key => ({
  ...
  // Merge with Local Storage
  const localReports = JSON.parse(localStorage.getItem('local_team_cd_daily_reports') || '[]');
  return [...reports, ...localReports];
};

export const createTeamCDDailyReport = async (report: Omit<TeamCDDailyReport, 'id'>) => {
  try {
    const response = await fetch(`${FIREBASE_URL}/team_cd_daily_reports.json`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    const newReport = { 
      ...report, 
      id: `local_cd_${Date.now()}`,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
    localReports.push(newReport);
    localStorage.setItem('local_team_cd_daily_reports', JSON.stringify(localReports));
    return { name: newReport.id };
  }
};

export const updateTeamCDDailyReport = async (id: string, report: Partial<TeamCDDailyReport>) => {
  if (id.startsWith('local_')) {
    const localReports = JSON.parse(localStorage.getItem('local_team_cd_daily_reports') || '[]');
    const index = localReports.findIndex((r: TeamCDDailyReport) => r.id === id);
    if (index !== -1) {
      localReports[index] = { 
        ...localReports[index], 
        ...report,
  ...
  try {
    const response = await fetch(`${FIREBASE_URL}/team_cd_daily_reports/${id}.json`, {
      method: 'PATCH',
      body: JSON.stringify({
        ...report,
        updatedAt: new Date().toISOString()
    ...
    if (!response.ok) throw new Error('Failed to update team CD daily report');
    return await response.json();
  } catch (error) {
    console.error("Update failed", error);
    throw error;
...
export const deleteTeamCDDailyReport = async (id: string) => {
  if (id.startsWith('local_')) {
    const localReports = JSON.parse(localStorage.getItem('local_team_cd_daily_reports') || '[]');
    const filtered = localReports.filter((r: TeamCDDailyReport) => r.id !== id);
    localStorage.setItem('local_team_cd_daily_reports', JSON.stringify(filtered));
    return true;
  }
  ...
  try {
    ...
    if (!response.ok) throw new Error('Failed to delete team CD daily report');
    return true;
  } catch (error) {
    console.error("Delete failed", error);
    throw error;
...
export const updateExpense = async (id: string, expense: Partial<Expense>) => {
  if (id.startsWith('local_')) {
    const localExpenses = JSON.parse(localStorage.getItem('local_expenses') || '[]');
    const index = localExpenses.findIndex((e: Expense) => e.id === id);
    if (index !== -1) {
      localExpenses[index] = {
        ...localExpenses[index],
        ...expense,
      ...
      localStorage.setItem('local_expenses', JSON.stringify(localExpenses));
      return localExpenses[index];
    }
    return null;
  }

  try {
    const response = await fetch(`${FIREBASE_URL}/expenses/${id}.json`, {
      method: 'PATCH',
      body: JSON.stringify({
        ...expense,
        updatedAt: new Date().toISOString()
...
export const deleteExpense = async (id: string) => {
  if (id.startsWith('local_')) {
    const localExpenses = JSON.parse(localStorage.getItem('local_expenses') || '[]');
    const filtered = localExpenses.filter((e: Expense) => e.id !== id);
