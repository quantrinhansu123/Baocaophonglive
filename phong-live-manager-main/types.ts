...
export interface TeamCDReportData {
  id?: string;
  date: string; // Ngày tháng (hoặc khoảng ngày)
  productId?: string; // ID sản phẩm
  productName?: string; // Tên sản phẩm
  storeId: string; // ID cửa hàng
  storeName?: string; // Tên cửa hàng
  totalGMV: number; // Tổng GMV (GMV 总额)
  videoGMV: number; // GMV Video (视频 GMV)
  videoOrders: number; // Số lượng đơn video (订单数量(视频))
  livestreamGMV: number; // GMV Livestream (直播 GMV)
  livestreamOrders: number; // Số lượng đơn livestream (订单数量(直播))
  newKOCVideoCount: number; // Số lượng KOC hợp tác video (mới) (合作视频的 KOC 数量(新增))
  newKOCLivestreamCount: number; // Số lượng KOC hợp tác livestream (mới) (合作直播的 KOC 数量(新增))
}
...
export interface TeamCDDailyReport {
  id?: string;
  stt?: number; // STT (Serial Number)
  date: string; // DATE (日期)
  tiktokStoreActivity?: string; // Hoạt động Tiktok/Cửa hàng (有活动的日子)
  shift?: string; // CA (班 - Shift)
  time?: number; // THỜI GIAN (时间 - Time in hours)
  salary?: number; // LƯƠNG (工作时间工资 - Salary)
  account?: string; // ACCOUNT (账号)
  pic?: string; // PIC (直播 - Livestream)
  products?: ProductItem[]; // Danh sách sản phẩm với số lượng
  // Legacy fields for backward compatibility
  purpleDetergentRevenue?: number;
  purpleDetergentOrders?: number;
  pinkDetergentRevenue?: number;
  pinkDetergentOrders?: number;
  otherProductsRevenue?: number;
  otherProductsOrders?: number;
  sum?: number; // SUM (营业额总和 - Total Revenue)
  admin?: string; // ADMIN (运营)
  session?: 'SANG' | 'CHIEU' | 'TOI'; // BUỔI (班次 - Session/Shift)
  dataScreenshot?: string; // DỮ LIỆU ẢNH (数据截图 - Data Screenshot URL)
  storeId?: string; // ID cửa hàng
  createdAt?: string; // Ngày tạo
  updatedAt?: string; // Ngày cập nhật
}
...
export interface ExpenseItem {
  costType: string; // Loại chi phí: 'LƯƠNG', 'VĂN PHÒNG', 'BẾP', 'CSKH', 'KHO', 'KHÁC'
...
export interface Expense {
  ...
  expenses: ExpenseItem[]; // Danh sách các chi phí
  isUrgent?: boolean; // DUYỆT GẤP (紧急审批)
  payer?: string; // Người chi (付款人)
  receiver?: string; // Người nhận (收款人)
  paymentVoucher?: string; // CHỨNG TỪ THANH TOÁN (付款凭证) - Base64 image
  accounting?: string; // Hạch toán (会计)
  periodType?: 'YEAR' | 'MONTH'; // THEO NĂM, THEO THÁNG (按年/按月)
  periodValue?: string; // Giá trị kỳ (năm hoặc tháng) - ví dụ: "2024" hoặc "2024-01"
  maintenanceProcess?: string; // QUY TRÌNH BẢO TRÌ (维护流程)
